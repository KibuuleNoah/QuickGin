...
// @Accept json
// @Produce json
// @Param body body forms.RefreshTokenForm true "Auth"
...
func (ctl *AuthController) RefreshToken(c *gin.Context) {
	...
	token, err := ctl.cfg.Asvc.RefreshTokens(form.Token)
	if err != nil {
...
func (ctl *AuthController) AuthLogout(c *gin.Context) {
	au, err := ctl.cfg.Asvc.ExtractTokenMetadata(c.Request)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"message": "User not logged in"})
		return
	}

	delErr := ctl.cfg.Asvc.DeleteAuth(au.AccessUUID)
	if delErr != nil {
