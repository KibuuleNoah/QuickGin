...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  // src/ts/stores/auth.store.ts
  function getStoredTokens() {
    const raw2 = localStorage.getItem("auth-creds");
  ...
  module_default.store("ui", {
    ...
    notify(message, type = "info", durationMs = 5500) {
  ...
  // src/ts/stores/dashboard.store.ts
  var BASE_VIEWS = ["Home", "Settings", "Analytics", "Review Content"];
  module_default.store("dashboard", {
    activeView: "Home",
    viewNavTree: ["Home"],
  ...
  // src/ts/lib/helpers.ts
  function getRemainingCooldown(expiresAt) {
  ...
  function validateIdentifier(input, authWith) {
    input = input.trim();
  ...
  function createNavTree(initialItem, storageKey) {
    const saved2 = localStorage.getItem(storageKey);
    let tree = saved2 ? JSON.parse(saved2) : [initialItem];
    function getTree() {
      return tree;
    }
    function save() {
  ...
  // src/ts/components/auth/auth-flow.component.ts
  module_default.data("authFlow", () => ({
  ...
  async function fetchWithAuth(method, path, body, retry = false) {
  ...
  var api = {
    async request(method, path, body) {
      try {
        ...
        const error2 = !res.ok ? res.json().then((err) => err?.message || "Something went Wrong!") : Promise.resolve("");
        const data2 = !error2 && res.status !== 204 ? await res.json().catch(() => null) : null;
        return { data: data2, error: error2, status: res.status, ok: res.ok };
      } catch (err) {
        return {
          data: null,
          status: 0,
  ...
  // src/ts/components/auth/auth-entry.component.ts
  module_default.data("authEntry", () => ({
    mode: "email",
    identifierErr: "",
    loading: false,
    get identifier() {
      return module_default.store("auth").identifier;
    },
    setIdentifier(val) {
      module_default.store("auth").setIdentifier(val);
    },
    get canSubmit() {
    ...
    switchMode(m) {
      this.mode = m;
      this.setIdentifier("");
      module_default.store("auth").setAuthWith(m);
      this.identifierErr = "";
    },
    validateOnBlur() {
      const auth = module_default.store("auth");
      if (!validateIdentifier(this.identifier, auth.authWith)) {
        this.identifierErr = auth.authWith === "mobile" ? "Invalid mobile number format!!, check again!" : "Invalid email address format!!, check again!";
      } else {
        this.identifierErr = "";
      }
    },
    async submit() {
      ...
      this.loading = false;
      if (!res.ok) {
        module_default.store("ui").notify(
  ...
  // src/ts/components/auth/auth-otp.component.ts
  module_default.data("authOtp", () => ({
    digits: ["", "", "", "", "", ""],
    ...
    init() {
      const expiry = new Date(Date.now() + 12e4).toISOString();
      module_default.store("auth").setOtpExpiry(expiry);
    ...
    async resend() {
      if (!this.timeLeft.isExpired) return;
      this.digits = ["", "", "", "", "", ""];
      module_default.store("ui").notify("OTP resent!", "info");
    },
    async verify() {
  ...
  // src/ts/components/auth/auth-profile.component.ts
  module_default.data("authProfile", () => ({
    username: "",
    name: "",
    loading: false,
    get canSubmit() {
  ...
  // src/ts/components/auth/auth-success.component.ts
  module_default.data("authSuccess", () => ({
    goToDashboard() {
      window.location.href = "/user";
    }
  }));

  // src/ts/components/dashboard/dashboard-layout.component.ts
  module_default.data("dashboardLayout", () => ({
    get activeView() {
      return module_default.store("dashboard").activeView;
    },
    get sidebarExpanded() {
      return module_default.store("dashboard").sidebarExpanded;
    },
    navigate(view) {
      module_default.store("dashboard").setView(view);
    },
    goBack() {
      module_default.store("dashboard").goBack();
    },
    get canGoBack() {
      return module_default.store("dashboard").canGoBack;
    },
  ...
  // src/ts/components/dashboard/retractable-sidebar.component.ts
  module_default.data("retractableSidebar", () => ({
    get expanded() {
      return module_default.store("dashboard").sidebarExpanded;
    },
    get activeView() {
    ...
    toggle() {
      module_default.store("dashboard").toggleSidebar();
    },
    isActive(label) {
      return this.activeView === label;
    },
    navigate(label) {
      module_default.store("dashboard").setView(label);
    }
  }));

  // src/ts/components/dashboard/mobile-nav.component.ts
  ...
  // src/ts/components/dashboard/top-nav-link.component.ts
  module_default.data("topNavLink", (label) => ({
    label,
    get isActive() {
      return module_default.store("dashboard").activeView === this.label;
    },
    navigate() {
      module_default.store("dashboard").setView(this.label);
    }
  }));
